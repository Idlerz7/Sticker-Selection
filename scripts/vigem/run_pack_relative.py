#!/usr/bin/env python3
"""Train or evaluate the independent pack-relative setwise VIGEM repair."""
...
from style_shapes.training import StyleShapesDataModule, build_final_only_trainer
...
def main() -> None:
    ...
    with command_record(args, args.artifact_root):
        ...
        if mode == "test":
            ...
            config["model_overrides"]["val_data_path"] = ""
            ...
            if visible_gpus <= 0:
                raise RuntimeError("formal pack-relative training needs a GPU")
        ...
        if completion_path.exists():
            ...
            compatible = (
                existing.get("vpd_membership_hash") == bank.membership_hash
                and existing.get("pack_membership_hash")
                == residual_manifest["pack_membership_hash"]
                and existing.get("pack_residual_bundle", {}).get("sha256")
                == sha256_file(config["pack_residual_bundle"])
            )
            if mode == "train":
            ...
            if not compatible:
                raise RuntimeError(
                    "refusing incompatible completed pack-relative output"
                )
        ...
        if mode == "train":
            ...
            if int(permutation["world_size"]) != world_size:
                raise RuntimeError(
                    "permutation world size differs from visible GPUs"
                )
            ...
            if errors:
                ...
                raise RuntimeError(
                    "pack-relative training failed completion contract: %s"
                    % "; ".join(errors)
                )
            ...
            result = {
                ...
                "pack_residual_bundle": {
                    "path": config["pack_residual_bundle"],
                    "sha256": sha256_file(config["pack_residual_bundle"]),
                },
                "init_checkpoint": {
                    "path": init_path,
                ...
                "group_gradient_policy": "detach_shared_bert",
                "r20_accessed": False,
            }
        elif mode == "test":
            if not checkpoint_path:
                raise ValueError("--checkpoint-path is required for test")
            if int(model_args.gpus) != 1:
                raise RuntimeError("formal pack-relative evaluation uses one GPU")
            ...
            score_path = Path(per_query_dir) / "rank_00_scores.json"
            report_path = output / "evaluation_report.json"
            baseline_path = str(
                args.baseline_scores
                or config.get("baseline_scores_path", "")
            )
            report = evaluate_pack_relative_scores(
                score_path=str(score_path),
                output_path=str(report_path),
                baseline_score_path=baseline_path,
                iterations=int(config.get("bootstrap_iterations", 10000)),
                seed=int(config.get("bootstrap_seed", 2021)),
            )
            result = {
                "status": "EVAL_COMPLETE",
                "schema_version": "vigem.pack_relative_eval.v1",
                "dataset": "stickerchat",
                "group_source": "vpd_pack",
                "vpd_membership_hash": bank.membership_hash,
                "pack_membership_hash": residual_manifest[
